...
def train():
    ...
    print("Number of epochs: {}".format(cfg.EPOCHS))
    ...
    train_generator = generator_from_data_path(img_names, gt_names, config=cfg, use_memory_cache=True)

    #make model parallel if specified
    if GPUS > 1:

        #use multigpu model checkpoint
        ckp_saver = ModelCheckpointMultiGPU(checkpoint_dir + "/model.{epoch:03d}-{loss:.2f}.hdf5", monitor='loss', verbose=0,
        ...
        parallel_model.fit_generator(train_generator, epochs=cfg.EPOCHS,
        ...
        #actually do the training
        squeeze.model.fit_generator(train_generator, epochs=cfg.EPOCHS,
                                        steps_per_epoch=nbatches_train, callbacks=cb)
