...
def sparse_to_dense(sp_indices, output_shape, values, default_value=0):
  ...
  array = cp.ones(output_shape) * default_value
...
def boxes_from_deltas(pred_box_delta, config):
    ...
    xmins = K.minimum(
        K.maximum(0.0, xmins), config.IMAGE_WIDTH_PROCESSING - 1.0)
    ymins = K.minimum(
        K.maximum(0.0, ymins), config.IMAGE_HEIGHT_PROCESSING - 1.0)
    xmaxs = K.maximum(
        K.minimum(config.IMAGE_WIDTH_PROCESSING - 1.0, xmaxs), 0.0)
    ymaxs = K.maximum(
        K.minimum(config.IMAGE_HEIGHT_PROCESSING - 1.0, ymaxs), 0.0)
...
def bbox_transform_single_box_to_CXCYWH(bbox):
    """convert a bbox of form [xmin, ymin, xmax, ymax] to [cx, cy, w, h] . Works
    for numpy array or list of tensors.
    """
    xmin, ymin, xmax, ymax = bbox
    ...
    out_box[0] = int(np.floor(xmin+w/2))
    out_box[1] = int(np.floor(ymin+h/2))
    out_box[2] = int(np.floor(w))
    out_box[3] = int(np.floor(h))

    return out_box

def bbox_transform_multiple_boxes_to_CXCYWH(bboxes):
    """convert bboxes of form [xmin, ymin, xmax, ymax] to [cx, cy, w, h]. Works
    for numpy array or list of tensors.
    """
    ...
    for bbox in bboxes:
        out_boxes.append(bbox_transform_single_box_to_CXCYWH(bbox))
...
def bbox_transform_single_box_float(bbox):
    """convert a bbox of form [cx, cy, w, h] to [xmin, ymin, xmax, ymax]. Works
    for numpy array or list of tensors.
    """
    cx, cy, w, h = bbox
    out_box = [[]]*4
    out_box[0] = float(cx-w/2)
    out_box[1] = float(cy-h/2)
    out_box[2] = float(cx+w/2)
    out_box[3] = float(cy+h/2)

    return out_box

def bbox_transform_multiple_boxes_float(bboxes):
    """convert bboxes of form [cx, cy, w, h] to [xmin, ymin, xmax, ymax]. Works
    for numpy array or list of tensors.
    """
    ...
    for bbox in bboxes:
        out_boxes.append(bbox_transform_single_box_float(bbox))
...
def boxes_from_deltas_np(pred_box_delta, config):
    ...
    xmins = np.minimum(
        np.maximum(0.0, xmins), config.IMAGE_WIDTH_PROCESSING - 1.0)
    ymins = np.minimum(
        np.maximum(0.0, ymins), config.IMAGE_HEIGHT_PROCESSING - 1.0)
    xmaxs = np.maximum(
        np.minimum(config.IMAGE_WIDTH_PROCESSING - 1.0, xmaxs), 0.0)
    ymaxs = np.maximum(
        np.minimum(config.IMAGE_HEIGHT_PROCESSING - 1.0, ymaxs), 0.0)
