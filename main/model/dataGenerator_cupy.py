...
def read_image_and_gt(img_files, gt_files, config, img_processor = None):
    ...
    #init tensor of images
    imgs = cp.zeros((config.BATCH_SIZE, config.IMAGE_HEIGHT_PROCESSING, config.IMAGE_WIDTH_PROCESSING, config.N_CHANNELS))
    ...
    for img_name, gt_name in zip(img_files, gt_files):
        ...
        #subtract means
        img = (img - cp.mean(img))/ cp.std(img)
    ...
    #concatenate ouputs
    input_mask = cp.array(input_mask)
    #box_input = cp.array(box_input)
    #box_delta_input = cp.array(box_delta_input)
    #labels = cp.array(labels)

    Y = cp.concatenate((input_mask, box_input,  box_delta_input, labels), axis=-1).astype(np.float32)
    ...
    return cp.asnumpy(imgs), cp.asnumpy(Y)
