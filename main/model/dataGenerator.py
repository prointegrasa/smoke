...
def prepare_images_raw_for_predict(images_raw, config, preprocessor):
    '''
    ...
    imgs = np.zeros((config.BATCH_SIZE, config.IMAGE_HEIGHT_PROCESSING, config.IMAGE_WIDTH_PROCESSING, config.N_CHANNELS))
...
def read_image(img_files, config, preprocessor):
    '''
    Transform images and send transformed image and label
    :param img_files: list of image files including the path of a batch
    :param gt_files: list of gt files including the path of a batch
...
#read_image_and_gt_count = 0
#read_image_and_gt_time = 0.0


#we could maybe use the standard data generator from keras?
def read_image_and_gt(img_files, gt_files, config, img_processor = None):
    '''
    Transform images and send transformed image and label
    :param img_files: list of image files including the path of a batch
    :param gt_files: list of gt files including the path of a batch
    ...
    config_ANCHOR_BOX = config.ANCHOR_BOX
    len_config_ANCHOR_BOX = len(config.ANCHOR_BOX)
    config_ANCHORS = config.ANCHORS
    ...
    for img_name, gt_name in zip(img_files, gt_files):
        ...
        # scale annotation
        # WP - done during preprocess
        #x_scale = config.IMAGE_WIDTH_PROCESSING / orig_w
        #y_scale = config.IMAGE_HEIGHT_PROCESSING / orig_h
        ...
        for i in range(len(bboxes_per_file)):
            ...
            overlaps = batch_iou(config_ANCHOR_BOX, bboxes_per_file[i])


            #achor box index
            aidx = len_config_ANCHOR_BOX
            ...
            if aidx == len_config_ANCHOR_BOX:
                dist = np.sum(np.square(bboxes_per_file[i] - config_ANCHOR_BOX), axis=1)
            ...
            delta = [0] * 4
            delta[0] = (box_cx - config_ANCHOR_BOX[aidx][0]) / config_ANCHOR_BOX[aidx][2]
            delta[1] = (box_cy - config_ANCHOR_BOX[aidx][1]) / config_ANCHOR_BOX[aidx][3]
            delta[2] = np.log(box_w / config_ANCHOR_BOX[aidx][2])
            delta[3] = np.log(box_h / config_ANCHOR_BOX[aidx][3])
    ...
    input_mask =  np.reshape(
            sparse_to_dense(
                mask_indices,
                [config.BATCH_SIZE, config_ANCHORS],
                [1.0] * len(mask_indices)),

            [config.BATCH_SIZE, config_ANCHORS, 1])

    box_delta_input =  sparse_to_dense(
            bbox_indices, [config.BATCH_SIZE, config_ANCHORS, 4],
            box_delta_values)
    ...
    #global read_image_and_gt_count
    #global read_image_and_gt_time

    #read_image_and_gt_count += 1
    #read_image_and_gt_time += t1 - t0

    #print("read_image_and_gt: ", read_image_and_gt_count," ",  read_image_and_gt_time)  # CPU seconds elapsed (floating point)
...
def read_image_and_gt_with_original(img_files, gt_files, config):
    ...
    imgs = np.zeros((config.BATCH_SIZE, config.IMAGE_HEIGHT_PROCESSING, config.IMAGE_WIDTH_PROCESSING, config.N_CHANNELS))
    imgs_only_resized = np.zeros((config.BATCH_SIZE, config.IMAGE_HEIGHT_PROCESSING, config.IMAGE_WIDTH_PROCESSING, config.N_CHANNELS))
...
def put_image_and_gt_cache(key,imgs,Y, cache_size_limit):

    global imgs_batch_cache
    global Y_batch_cache

    if len(imgs_batch_cache) < cache_size_limit:
        imgs_batch_cache[key] = imgs
        Y_batch_cache[key] = Y

def exists_image_and_gt_cache(key):
    global imgs_batch_cache

    return key in imgs_batch_cache

def get_image_and_gt_cache(key):

    global imgs_batch_cache
    global Y_batch_cache
    return imgs_batch_cache[key], Y_batch_cache[key]



@threadsafe_generator
def generator_from_data_path(img_names, gt_names, config, return_filenames=False, shuffle=False, use_memory_cache = False):
    """
    ...
    while 1:
        ...
        for _ in range(nbatches):
            ...
            try:
                ...
                if use_memory_cache:
                    if exists_image_and_gt_cache(i):
                        imgs, gts = get_image_and_gt_cache(i)
                    else:
                        img_names_batch = img_names[i:j]
                        gt_names_batch = gt_names[i:j]
                        imgs, gts = read_image_and_gt(img_names_batch, gt_names_batch, config)
                        # store in cache for use in next epochs
                        put_image_and_gt_cache(i, imgs, gts, config.BATCH_CACHE_SIZE_LIMIT)
                else:
                    img_names_batch = img_names[i:j]
                    gt_names_batch = gt_names[i:j]
                    imgs, gts = read_image_and_gt(img_names_batch, gt_names_batch, config)
...
@threadsafe_generator
def generator_from_data_path_with_processor(img_names, gt_names, config, img_processor, return_filenames=False, shuffle=False):
    """
    Generator that yields (X, Y)
    :param img_names: list of images names with full path
    :param gt_names: list of gt names with full path
    ...
    while 1:
        ...
        for _ in range(nbatches):
            ...
            try:
                ...
                imgs, gts = read_image_and_gt(img_names_batch, gt_names_batch, config, img_processor)
