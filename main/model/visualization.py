...
def visualize(model, generator, config):
    ...
    for images, y_true, images_only_resized in generator:
        ...
        if count >= nbatches:
            break
    try:
        return np.stack(all_boxes).reshape((-1, config.IMAGE_HEIGHT_PROCESSING, config.IMAGE_WIDTH_PROCESSING, 3))
    except:
        return np.zeros( (nbatches*config.BATCH_SIZE, config.IMAGE_HEIGHT_PROCESSING, config.IMAGE_WIDTH_PROCESSING, 3))
...
def visualize_dt_and_gt(images, y_true, y_pred, config):
    ...
    for i, img in enumerate(images):
        ...
        for j, det_box in enumerate(all_filtered_boxes[i]):
            ...
            det_box = utils.bbox_transform_single_box(det_box)
        ...
        for j, gt_box in enumerate(non_zero_boxes):
            ...
            #transform into xmin, ymin, xmax, ymax
            gt_box = utils.bbox_transform_single_box(gt_box)
