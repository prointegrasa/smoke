...
def squeezeDet_config(name):
    ...
    cfg.CLASS_NAMES = ['bottle', 'cigarettes', 'knife', 'sand', 'powder']
    ...
    #image properties
    cfg.IMAGE_WIDTH_PROCESSING           = 800
    cfg.IMAGE_HEIGHT_PROCESSING          = 800
    cfg.IMAGE_WIDTH_STORAGE              = 800
    cfg.IMAGE_HEIGHT_STORAGE             = 800
    ...
    cfg.FINAL_THRESHOLD       = 0.50

    
    cfg.ANCHOR_SEED = np.array([[  96.,  315.], [ 223., 75.], [ 100.,  135.],
                                [ 156., 141.], [  213., 100.], [  375.,  120.]])



    cfg.ANCHOR_PER_GRID       = len(cfg.ANCHOR_SEED)

    cfg.ANCHORS_HEIGHT = 50
    cfg.ANCHORS_WIDTH = 50
...
def set_anchors(cfg):
  ...
  center_x = np.reshape(
      np.transpose(
          np.reshape(
              np.array([np.arange(1, W+1)*float(cfg.IMAGE_WIDTH_PROCESSING)/(W+1)]*H*B),
              (B, H, W)
          ),
          (1, 2, 0)
      ),
  ...
  center_y = np.reshape(
      np.transpose(
          np.reshape(
              np.array([np.arange(1, H+1)*float(cfg.IMAGE_HEIGHT_PROCESSING)/(H+1)]*W*B),
              (B, W, H)
          ),
          (2, 1, 0)
      ),
