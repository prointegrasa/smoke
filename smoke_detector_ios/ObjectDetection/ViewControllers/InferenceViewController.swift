...
class InferenceViewController: UIViewController {
  ...
  @IBOutlet weak var ttaSwitch: UISwitch!
  ...
  var currentThreshold: Float = 0.6
  var ttaEnabled: Bool = true
  ...
  override func viewDidLoad() {
    ...
    thresholdStepper.isUserInteractionEnabled = true
    thresholdStepper.maximumValue = Double(10.0)
    thresholdStepper.minimumValue = Double(1.0)
    thresholdStepper.value = Double(currentThreshold * 10)
  ...
  @IBAction func onClickThresholdStepper(_ sender: Any) {
    
    delegate?.didChangeThresholdValue(to: thresholdStepper.value / 10)
    currentThreshold = Float(thresholdStepper.value / 10)
    thresholdValueLabel.text = String(format: "%.1f", currentThreshold)
  }
  
  @IBAction func onClickTTASwitch(_ sender: Any) {

    delegate?.didChangeTTAValue(to: ttaSwitch.isOn)
    ttaEnabled = ttaSwitch.isOn
  }
  
  func changeTTAButtonStateDueToContinuesDetection() {
    ttaSwitch.isEnabled = !ttaSwitch.isEnabled
  }
}
